...
export function AdminHeader() {
  ...
  return (
    <header className="h-16 bg-white/90 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs">
      {/* Global Search Bar matching Section 7 */}
      <div className="relative w-80">
        ...
        <input
          ...
          placeholder="Search users, workspaces, posts, jobs, invoices..."
          className="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-all"
        />

        {/* Global Search Results Overlay Modal */}
        {showSearchModal && (
          <div className="absolute top-12 left-0 w-96 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-2xl z-50 space-y-3">
            ...
            {searchQuery ? (
              ...
              <div className="text-xs text-slate-400 space-y-1">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  <button onClick={() => setSearchQuery('joshua')} className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-[11px] font-medium rounded-lg">joshua</button>
                  <button onClick={() => setSearchQuery('failed')} className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-[11px] font-medium rounded-lg">failed jobs</button>
                  <button onClick={() => setSearchQuery('inv_')} className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-[11px] font-medium rounded-lg">invoices</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 top-12 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-2xl z-50 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
        ...
        <div className="relative pl-3 border-l border-slate-200 dark:border-slate-800">
          ...
          {showProfileMenu && (
            <div className="absolute right-0 top-12 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 shadow-2xl z-50 space-y-2">
              <div className="p-2 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
                <div className="text-xs font-black text-slate-900 dark:text-slate-100">{admin?.email || 'admin@omnipost.com'}</div>
