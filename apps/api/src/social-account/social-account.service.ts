...
export class SocialAccountService {
  ...
  async getOAuthUrl(platformType: PlatformType): Promise<OAuthUrlResponse> {
    ...
    const facebookKey = process.env.FACEBOOK_CLIENT_ID || 'MOCK_CLIENT_ID';
    const linkedinKey = process.env.LINKEDIN_CLIENT_ID || 'MOCK_CLIENT_ID';

    const authUrls: Record<PlatformType, string> = {
      INSTAGRAM: `https://api.instagram.com/oauth/authorize?client_id=${instagramKey}&redirect_uri=${webUrl}/oauth/callback&response_type=code&scope=user_profile,user_media&state=${state}`,
      FACEBOOK: `https://www.facebook.com/v18.0/dialog/oauth?client_id=${facebookKey}&redirect_uri=${webUrl}/oauth/callback&state=${state}`,
      TIKTOK: `https://www.tiktok.com/v2/auth/authorize/?client_key=${tiktokKey}&response_type=code&scope=user.info.basic,video.publish&redirect_uri=${webUrl}/oauth/callback&state=${state}`,
      YOUTUBE: `https://accounts.google.com/o/oauth2/v2/auth?client_id=${googleKey}&redirect_uri=${webUrl}/oauth/callback&response_type=code&scope=https://www.googleapis.com/auth/youtube.upload&state=${state}`,
      X: `https://twitter.com/i/oauth2/authorize?client_id=${twitterKey}&redirect_uri=${webUrl}/oauth/callback&response_type=code&scope=tweet.read,tweet.write,users.read&state=${state}`,
      LINKEDIN: `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${linkedinKey}&redirect_uri=${webUrl}/oauth/callback&state=${state}`,
      THREADS: `https://threads.net/oauth/authorize?client_id=MOCK_CLIENT_ID&redirect_uri=${webUrl}/oauth/callback&response_type=code&state=${state}`,
      PINTEREST: `https://www.pinterest.com/oauth/?client_id=MOCK_CLIENT_ID&redirect_uri=${webUrl}/oauth/callback&response_type=code&state=${state}`,
      TELEGRAM: `https://telegram.org/auth?bot_id=MOCK_BOT_ID&origin=${webUrl}&return_to=${webUrl}/oauth/callback`,
